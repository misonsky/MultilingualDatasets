...
class Decoder(keras.Model):
    ...
    def call(self, x, hidden,encoder_outputs,context_mask):
        ...
        """
        x = self.embedding(x)
        hidden_vector = tf.reduce_sum(tf.stack(hidden,axis=0),axis=0)
        # word level attention
        context_output = []
        for utt in tf.unstack(encoder_outputs,axis=1):
            context_output.append(self.word_level_attn(hidden_vector, utt))
        ...
        context_output = self.utt_level_attn(hidden_vector,context_output)
...
class HRAN(keras.Model):
    ...
    def call(self,features,training=True):
        ...
        for utt in src:
            ...
            _,state = self.encoder(utt,mask=mask)#(num * bid) * batch * d
        ...
        dec_hidden = self.noraml_initializer(shape=(self.config.num_layers,context_mask.shape[0], utterances.shape[-1]))
    ...
    def BeamDecoder(self,features):
        ...
        for utt in src:
            ...
            mask = tf.cast(mask,dtype=tf.zeros(1).dtype)
            context_mask.append(tf.reduce_sum(mask,axis=1))
            _,state = self.encoder(utt,mask=mask)
            state = tf.transpose(tf.stack(state,axis=0),perm=[1,0,2])
            utterances.append(tf.reshape(state,shape=[state.shape[0],state.shape[1] * state.shape[-1]]))
        context_mask = tf.cast(tf.stack(context_mask,axis=1),dtype=tf.bool)
        context_mask = tf.cast(context_mask,dtype=tf.zeros(1).dtype)
        ...
        states = {"dec_hidden":dec_hidden,"utterances":utterances,"mask":context_mask}
