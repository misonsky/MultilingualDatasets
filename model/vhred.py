...
class VHRED(keras.Model):
    def __init__(self,vocab_size,embedding_dim,matrix,config,PROM=0,EOS=0,PAD=0):
        ...
        if config.bow:
            self.bow_p = keras.layers.Dense(config.d_model)
            self.bow_h = keras.layers.Dense(config.d_model,activation=tf.nn.tanh)
    ...
    def bow_loss(self,target_bow, bow_logits):
        ...
        loss = loss /tf.reduce_sum(target_bow)
        return loss
    def compute_bow_loss(self,position,tbow,tatget_hidden,z_sent):
        """
        parameters:
            position: batch * seq
            t_states: batch * hidden
            tbow: batch * seq
            z_sent: batch * hidden
        """
        bp = tf.tile(tf.expand_dims(tf.range(position.shape[0]),axis=1),multiples=[1,position.shape[1]])
        position = tf.concat([tf.expand_dims(bp,axis=-1),tf.expand_dims(position,axis=-1)],axis=-1)#batch * seq * 2
        position = tf.reshape(position,shape=[position.shape[0]*position.shape[1],position.shape[-1]])
        tbow = tf.reshape(tbow,shape=[tbow.shape[0] * tbow.shape[1]])
        bow_label = tf.zeros(shape=[z_sent.shape[0],self.output_size],dtype=self.dytype)#batch * vocab
        tbow = tf.cast(tbow,dtype=self.dytype)
        bow_label = tf.tensor_scatter_nd_update(bow_label, position, tbow,name="bow_label") #(batch * vocab_size)
        bow_inputs = tf.concat([tf.reduce_sum(tatget_hidden,axis=0),z_sent],axis=-1)
        bow_inputs = self.bow_h(bow_inputs)   #batch * hidden
        logits = self.bow_predict(bow_inputs)   #batch * vocab
        loss = self.bow_loss(bow_label, logits)
        return loss
    ...
    def call(self,features,training=True):
        ...
        if self.config.bow:
            bow_loss= self.compute_bow_loss(tgt[:,1:],tbow[:,1:],tgt_context,z_sent)
            loss  += bow_loss
    ...
    def BeamDecoder(self,features):
        ...
        context_mask = tf.cast(context_mask,dtype=tf.zeros(1).dtype)
        utterances = tf.stack(utterances, axis=1)#batch * max_turn * hidden
        utterances = tf.unstack(utterances,axis=0)
        with open("vhred_ru.txt","w",encoding="utf-8") as f:
            for i in range(len(utterances)):
                utts= tf.unstack(utterances[i],axis=0)
                for j in range(len(utts)):
                    name ="u_%d_%d"%(i,j)
                    items = utts[j].numpy()
                    items = items.tolist()
                    items = [str(item) for item in items]
                    f.write(name+" "+" ".join(items)+"\n")
        _, enc_context = self.encoder2(utterances,mask=context_mask,useEmbedding=False)
        enc_context = tf.stack(self.stackStates(enc_context),axis=0)# num_layer  * batch * d
        z_sent, _,_, _ = self.variablelayer(context_outputs = tf.reshape(enc_context,shape=[enc_context.shape[1],enc_context.shape[-1]*self.config.num_layers]), 
                                            tag_context=None,
                                            training=False)
        z_sent = tf.tile(tf.expand_dims(z_sent, axis=0),multiples=[self.config.num_layers,1,1])
        enc_context = tf.concat([enc_context, z_sent],axis=2) #batch * (hidden + 
        enc_context = self.denseProj(enc_context)
        dec_hidden = tf.unstack(enc_context, axis=0)
